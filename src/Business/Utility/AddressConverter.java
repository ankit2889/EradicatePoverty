...
public class AddressConverter {
 ...
 public static void getGeographicDetails(EnrollmentWorkRequest workRequest,String villageName,String districtName) throws IOException
 {
  
  GoogleResponse res = new AddressConverter().convertToLatLong(villageName+","+districtName);
  if(res.getStatus().equals("OK"))
  {
   for(Result result : res.getResults())
   {
   workRequest.getLocation().setLattitude(Double.parseDouble(result.getGeometry().getLocation().getLat()));
   workRequest.getLocation().setLongitude(Double.parseDouble(result.getGeometry().getLocation().getLng()));
